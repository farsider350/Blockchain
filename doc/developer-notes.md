...
autx Core is a multithreaded application, and deadlocks or other multithreading bugs
...
However, in open source software such as autx Core, where everyone uses
...
pay attention to for reviewers of autx Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: autx Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against autx Core so that they can be integrated
