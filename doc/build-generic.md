GENERIC BUILD NOTES
====================
Some notes on how to build autx Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build autx with system-wide installed dependencies
...
------------------------------------
Building the dependencies and autx Core requires some essential build tools to be installed before. Please see
[build-unix](build-unix.md), [build-osx](build-osx.md) and [build-windows](build-windows.md) for details.

Building dependencies
---------------------
autx inherited the `depends` folder from Bitcoin, which contains all dependencies required to build autx. These
dependencies must be built before autx can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build autx Core. Caching of build results will ensure that only
...
not done, build failures might occur when building autx.
...
ccache
------
`./configure` of autx Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
