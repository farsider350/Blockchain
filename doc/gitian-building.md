...
*Setup instructions for a Gitian build of autx Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the autx
...
- [Building autx Core](#building-autx-core)
...
Clone the git repositories for autx Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/genixpay/autx
git clone https://github.com/genixpay/gitian.sigs.git
```
...
in the autx Core repository under 'Fetch and create inputs' to install sources which require
...
To build autx Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the autx Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/autx/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/genixpay/autx
...
For example:
```bash
URL=https://github.com/crowning-/autx.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit autx=${COMMIT} --url autx=${URL} ../autx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit autx=${COMMIT} --url autx=${URL} ../autx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit autx=${COMMIT} --url autx=${URL} ../autx/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the autx git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../autx/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/genixpay/autx-detached-sigs.git

BTCPATH=/some/root/path/autx
SIGPATH=/some/root/path/autx-detached-sigs

./bin/gbuild --url autx=${BTCPATH},signature=${SIGPATH} ../autx/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/autx-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/autx-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/autx-osx-build.assert
```
...
[autx/gitian.sigs](https://github.com/genixpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
