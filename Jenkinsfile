...
// which are part of the autx repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="autx-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          if (hasCache) {
            sh "cd /autx-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /autx-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /autx-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /autx-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /autx-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /autx-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /autx-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
