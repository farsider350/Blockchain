...
# autx_hash
RUN git clone https://github.com/farsider350/autx-core_hash
RUN cd autx_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} autx
RUN useradd -u ${USER_ID} -g autx -s /bin/bash -m -d /autx autx
...
RUN mkdir /autx-src && \
  ...
  chown $USER_ID:$GROUP_ID /autx-src && \
