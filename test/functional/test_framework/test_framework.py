...
class BitcoinTestFramework(object):
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave autxds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop autxds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing autxd/autx-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: autxds were not stopped and may still be running")
    ...
    def stop_node(self, i, wait=0):
        """Stop a autxd test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple autxd test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'autxd exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "autxd should have exited with an error"
                else:
                    assert_msg = "autxd should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as autxd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, extra_args=None, stderr=None):
        ...
        if create_cache:
            ...
            # Create cache directories, run autxds:
            self.set_genesis_mocktime()
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i)
                args = [os.getenv("autxD", "autxd"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0", "-mocktime="+str(GENESISTIME)]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in autx.conf
...
class autxTestFramework(BitcoinTestFramework):
    def set_autx_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
...
class ComparisonTestFramework(BitcoinTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "autxd"),
                          help="autxd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "autxd"),
                          help="autxd binary to use for reference nodes (if any)")
