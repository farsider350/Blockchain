...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s autx too low! (Should be %s autx)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s autx too high! (Should be %s autx)" % (str(fee), str(target_fee)))
...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "autx.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "autx.conf")):
        with open(os.path.join(datadir, "autx.conf"), 'r', encoding='utf8') as f:
            for line in f:
