...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from gewalletinfo RPC and `autx-cli getwalletinfo`")
        cli_get_info = self.nodes[0].cli.getwalletinfo()
        rpc_get_info = self.nodes[0].getwalletinfo()

        assert_equal(cli_get_info, rpc_get_info)

        self.log.info("Compare responses from getblockchaininfo RPC and `autx-cli getblockchaininfo`")
