...
from test_framework.mininode import autxhash
...
def autxhash_helper(b):
    return encode(autxhash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def setup_nodes(self):
        ...
        # Check that autx has been built with ZMQ enabled
        ...
        if not config["components"].getboolean("ENABLE_ZMQ"):
            raise SkipTest("autxd has not been built with zmq enabled.")
    ...
    def _zmq_test(self):
        ...
        assert_equal(genhashes[0], autxhash_helper(body[:80]))
        ...
        for x in range(n * 4):
            ...
            if topic == b"hashblock":
                ...
                blockcount += 1
            if topic == b"rawblock":
                zmqRawHashed.append(autxhash_helper(body[:80]))
                msgSequence = struct.unpack('<I', msg[-1])[-1]
