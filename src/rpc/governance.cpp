...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 autx irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on autxcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in autx.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("autx.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in autx.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* autx features */
    { "autx",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "autx",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "autx",               "gobject",                &gobject,                true,  {} },
    { "autx",               "voteraw",                &voteraw,                true,  {} },
