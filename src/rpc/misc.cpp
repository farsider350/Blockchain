...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total autx balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            " - \"autx\" activates all autx-specific categories at once;\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "autx")
            + HelpExampleRpc("debug", "autx+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given autx address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The autx address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The autx address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: autx address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are autx addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) autx address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The autx address to use for the signature.\n"
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The difference of twigs\n"
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The number of twigs of the output\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"balance\"  (string) The current balance in twigs\n"
            "  \"received\"  (string) The total number of twigs received (including change)\n"
            "}\n"
            "\nExamples:\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* autx features */
    { "autx",               "mnsync",                 &mnsync,                 true,  {} },
    { "autx",               "spork",                  &spork,                  true,  {"value"} },
