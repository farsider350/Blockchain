// Copyright (c) 2018-2020 The autx Core developers
...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The autx address to send the collateral to.\n"
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The autx address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            "%d. \"payoutAddress\"            (string, required) The autx address to use for masternode reward payments.\n"
        },
...
// Allows to specify autx address or priv key. In case of autx address, the priv key is taken from the wallet
...
void protx_register_fund_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 10000 autx\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > paramIdx + 6) {
        ...
        if (!fundAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid autx address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() >= 6) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid autx address: ") + request.params[5].get_str());
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid autx address: ") + request.params[4].get_str());
