...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all autx related functionality
...
/*
    ...
    allow endless transaction that would bloat autx and make it unusable. To
...
bool CPrivateSendServer::IsInputScriptSigValid(const CTxIn& txin)
{
    ...
    if (nTxInIndex >= 0) { //might have to do this one input at a time?
        ...
        // TODO we're using amount=0 here but we should use the correct amount. This works because autx ignores the amount while signing/verifying (only used in Bitcoin/Segwit)
...
{
    if (fLiteMode) return;        // disable all autx specific functionality
