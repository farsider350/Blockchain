...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::autx);
    ...
    if (!settings.contains("nPrivateSendAmount")) {
        ...
        if (!settings.contains("nAnonymizeautxAmount"))
            settings.setValue("nPrivateSendAmount", DEFAULT_PRIVATESEND_AMOUNT);
        else
            settings.setValue("nPrivateSendAmount", settings.value("nAnonymizeautxAmount").toInt());
    }
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in autx.cpp during shotdown to also remove the window positions
