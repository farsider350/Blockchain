...
const char * const BITCOIN_CONF_FILENAME = "autx.conf";
const char * const BITCOIN_PID_FILENAME = "autxd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\autxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\autxCore
    // Mac: ~/Library/Application Support/autxCore
    // Unix: ~/.autxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "autxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/autxCore";
#else
    // Unix
    return pathRet / ".autxcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty autx.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override autx.conf
